...
import numpy as np
import random
from CustomMetrics import PRF4TgtCls
...
#########################first step######################
args.class_num = 2
bin_y_train = [int(y != 0) for y in y_train]
bin_y_valid = [int(y != 0) for y in y_valid]
bin_y_test =  [int(y != 0) for y in y_test]

xgb_based = XGB_based()
xgb_based.buildCountVectorizer(X_train+X_valid+X_test)

xgb_based.fit(X_train, bin_y_train, X_valid, bin_y_valid)

bin_y_pred, y_pred_prob = xgb_based.classify(X_test)


#########################second step######################
args.class_num = 3
X_train = [x for x, y in zip(X_train, y_train) if y != 0]
y_train = [(y-1) for y in y_train if y != 0]

X_valid = [x for x, y in zip(X_valid, y_valid) if y != 0]
y_valid = [(y-1) for y in y_valid if y != 0]
...
mul_y_pred, y_pred_prob = xgb_based.classify(X_test)
