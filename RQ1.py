...
for dataset, path in datasets.items():
    for method, pyfile in methods.items():
        for round in range(rounds):
            ...
            log_file = f"{log_folder}/binary_{dataset}_{method}_{round}.txt"
...
for dataset, _ in datasets.items():
    ...
    for method, _ in methods.items():
        ...
        lines = ""
        for round in range(rounds):
            temp_metric_dic = process_file(log_file=f"{log_folder}/binary_{dataset}_{method}_{round}.txt", metric_list=metric_list)
